...
/* USER CODE BEGIN PV */
volatile RobotData_t g_robot_data;
volatile ButtonCommand_t g_btn_cmd_queue = BTN_CMD_NONE;
#define RX_BUFFER_SIZE 256
uint8_t rx_byte;                // 1바이트 수신 임시 변수
uint8_t rx_buffer[RX_BUFFER_SIZE]; // 한 문장 저장 버퍼
uint8_t rx_index = 0;
/* USER CODE END PV */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void Parse_Robot_Packet(char* packet)
{
    // strtok를 사용하기 위해 임시 포인터 사용
    char *token;
    char *rest = packet;
    ...
    g_robot_data.current_state = (RobotState_t)mode_val;
    ...
    token = strtok_r(rest, "@", &rest);
    if(token == NULL) return;
    g_robot_data.battery_percent = atoi(token);

    // 4. 호출자 (String)
    token = strtok_r(rest, "@", &rest);
    if(token != NULL) strncpy((char*)g_robot_data.caller_name, token, 63);
    else g_robot_data.caller_name[0] = '\0';

    // 5. 출발지 (String)
    token = strtok_r(rest, "@", &rest);
    if(token != NULL) strncpy((char*)g_robot_data.start_loc, token, 63);
    else g_robot_data.start_loc[0] = '\0';

    // 6. 도착지 (String) - 마지막 토큰은 개행문자(\n) 처리 주의
    token = strtok_r(rest, "@", &rest);
    if(token != NULL) {
        // 개행 문자 제거
        token[strcspn(token, "\r\n")] = 0;
        strncpy((char*)g_robot_data.dest_loc, token, 63);
    }
    else g_robot_data.dest_loc[0] = '\0';

    // UI 업데이트 플래그 설정
    g_robot_data.is_updated = true;
}

// UART 수신 인터럽트 콜백 (1바이트씩 받을 때마다 호출)
    ...
    if (huart->Instance == USART1)
    {
        ...
        if (rx_index < RX_BUFFER_SIZE - 1) {
            ...
            if (rx_buffer[rx_index] == '\n') {
                ...
                Parse_Robot_Packet((char*)rx_buffer); // 파싱 시작
                rx_index = 0; // 인덱스 초기화 (다음 패킷 준비)
            } else {
                rx_index++;
            }
        } else {
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // UART 인터럽트 수신 시작 (1바이트)
  HAL_UART_Receive_IT(&huart1, &rx_byte, 1);

  // 초기 데이터 더미 값 설정 (디버깅용)
  g_robot_data.current_state = STATE_WAITING;
  g_robot_data.speed_kmh = 0;
  g_robot_data.battery_percent = 100;
  strcpy((char*)g_robot_data.caller_name, "");
  /* USER CODE END 2 */
